...
<div class="row">
    <div class="col-md-8">
        <div class="box box-primary">
            ...
            <form role="form" action="{{ route('applicationroles-store') }}?index=true" method="post" id="frm">
                {{ csrf_field() }}
                <div class="box-body">
                    ...
                    <div class="form-group">
                        <label>Module truy cập <small class="text-danger text"> (*)</small>: </label><br>
                            @foreach($applicationmodules as $item)
                                ...
                                <input type="checkbox" name="modules_{{$item->id}}" value="{{$item->id}}" {{(old('modules_' . $item->id) == $item->id) ? 'checked' : '' }}> <br> 
                            @endforeach
                    </div>
                    <div class="form-group">
                        <label>Ghi chú : </label>
